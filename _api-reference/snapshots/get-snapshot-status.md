...
- `snapshot.max_shards_allowed_in_status_api`(Dynamic, integer): The maximum number of shards that can be included in the snapshot status API response. Not applicable for [shallow V2 snapshots]({{site.url}}{{site.baseurl}}/tuning-your-cluster/availability-and-recovery/remote-store/snapshot-interoperability#shallow-snapshot-v2) Default is `200000`. 
In case of [shallow V2 snapshots]({{site.url}}{{site.baseurl}}/tuning-your-cluster/availability-and-recovery/remote-store/snapshot-interoperability#shallow-snapshot-v2), the total number and size of files are returned as 0, owing to the large number of shards in the snapshots. 
{: .warning}
